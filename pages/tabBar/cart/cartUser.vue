					...
					<view class="goods-info" @tap="toGoods(row)">
						...
						<view class="info">
							...
							<view class="price-number">
								<view class="price">{{row.age}} years old</view>
							</view>
						</view>
					</view>
				</view>
...
<script>
	...
	export default {
		...
		onLoad() {
			this.department = uni.getStorageSync('department_doc')
			this.getUserList_doc()
		...
		methods: {
			getUserList_doc(){
				...
				uni.request({
					url:'http://52.77.228.143:8080/getUserList',
					data:{
			...
			//商品跳转
			toGoods(e){
				uni.setStorageSync('userid',e.id)
				// uni.showToast({title: '商品'+e.id,icon:"none"});
				uni.navigateTo({
					url: 'cartZYdoc/cartZYdoc' 
				});
			},
			//跳转确认订单页面
			toConfirmation(){
