	...
	<view>
		...
		<view class="footer" :style="{}">
			...
			<view class="settlement">
				...
				<!-- <view class="btn" @tap="toConfirmation">提交诊断</view> -->
			</view>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		onLoad() {
			this.department = uni.getStorageSync('department_nurse')
		...
		methods: {
			getMedicalRecord(){
				...
				uni.request({
					url:'http://52.77.228.143:8080/getMedicalRecordByDepartment2',
					data:{
			...
			//商品跳转
			toGoods(e){
				uni.setStorageSync('goodsId',e.id)
				// uni.showToast({title: '商品'+e.id,icon:"none"});
				uni.navigateTo({
					url: '../../goods/medicalRecordGoods' 
				});
			},
			//跳转确认订单页面
			toConfirmation(){
